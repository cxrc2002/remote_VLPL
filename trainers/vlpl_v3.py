...
class Prompt_Argument(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.attpool = AttentionPool2d(8, self.ctx_dim, 8, 1024)
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        ctx_vectors = torch.empty(5, n_ctx, ctx_dim, dtype=dtype)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        out4 = self.prompt_argument(out4) # (n, ctx_dim)

        out1 = out1 / out1.norm(dim=-1, keepdim=True) 
        out2 = out2 / out2.norm(dim=-1, keepdim=True) 
        out3 = out3 / out3.norm(dim=-1, keepdim=True) 
        out4 = out4 / out4.norm(dim=-1, keepdim=True) 

        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        all_image_features = torch.stack([image_features, out1, out2, out3, out4], dim=1)
        prompts = self.prompt_learner() # prompts (n_cls, 5, *, dim)
        ...
        tokenized_prompts = tokenized_prompts.unsqueeze(1).expand(-1, 5, -1)
        ...
        for i in range(text_features.size(1)):
            logit = torch.einsum("nd,kd-> nk", all_image_features[:, i, :], text_features[:, i, :])
            logit = logit_scale*logit
            logits.append(logit)
        logits = torch.stack(logits, dim=1) #n,m,k
